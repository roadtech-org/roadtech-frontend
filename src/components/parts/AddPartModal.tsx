...
export function AddPartModal({ onClose, onSuccess }: AddPartModalProps) {
  const {
    register,
    handleSubmit,
    formState: { errors },
  ...
  const addMutation = useMutation({
    mutationFn: partsProviderApi.addPart,
    onSuccess: () => {
      toast.success('Part added successfully! 🎉');
      onSuccess();
    },
    onError: (error: any) => {
      const message = error.response?.data?.message || 'Failed to add part';
      toast.error(message);
    },
  });
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-lg max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        <div className="sticky top-0 bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between">
          <h2 className="text-xl font-bold text-gray-900">Add New Part</h2>
          <button
            onClick={onClose}
