...
export function PartsProviderDashboard() {
  ...
  const totalValue =
    parts?.reduce((sum, part) => sum + part.price * part.stock, 0) || 0;
  const totalItems =
    parts?.reduce((sum, part) => sum + part.stock, 0) || 0;
    ...
    <div className="space-y-6">
      ...
      {!profile?.isVerified && (
        <div className="rounded-lg border border-yellow-300 bg-yellow-50 p-4 text-sm text-yellow-800">
          Your shop is currently under admin verification.
          You can view your dashboard, but actions will be enabled after approval.
        </div>
      )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <StatCard
          icon={<Store />}
          label="Status"
          value={profile?.isOpen ? 'Open' : 'Closed'}
          active={profile?.isOpen}
        />
        <StatCard
          icon={<Package />}
          label="Total Parts"
